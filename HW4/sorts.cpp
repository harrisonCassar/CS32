...
int main()
{
      ...
      // destruction time.
    {
      ...
      vector<Store*> ptrsToStores;

      vector<Store>::iterator it = auxStores.begin();

      while (it != auxStores.end())
      {
        ptrsToStores.push_back(&(*it));

        it++;
      }

      sort(ptrsToStores.begin(),ptrsToStores.end(),compareStorePtr);

      it = stores.begin();
      vector<Store*>::iterator it_ptrs = ptrsToStores.begin();
